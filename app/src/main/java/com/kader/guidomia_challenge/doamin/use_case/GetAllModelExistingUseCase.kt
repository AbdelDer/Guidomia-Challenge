...
private const val TAG = "GetAllModelExistingUseCase"

class GetAllModelExistingUseCase @Inject constructor(private val repository: CarRepository) {

    operator fun invoke(): Flow<Resource<List<String>>> {
        Log.d(TAG, "invoke: method called")
        return repository.getAllModelsExisting()
    }
}